...
petting-failure-derelict-cyborg = Вы протягиваете руку, чтобы погладить { ($target) }, но { POSS-ADJ($target) } ржавый и зазубренный внешний вид заставляет вас передумать.
petting-failure-station-ai = Вы протягиваете руку, чтобы погладить { ($target) }, но { SUBJECT($target) } { CONJUGATE-BASIC($target, "zap", "zaps") } отводит вашу кисть руки.
petting-success-station-ai-others = { CAPITALIZE(THE($user)) } гладит { THE($target) } по холодному, квадратному экрану { POSS-ADJ($target) }.
